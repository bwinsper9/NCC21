...
if num_guests and selected_recipes:
    ...
    if st.button("Download Shopping List PDF"):
        shopping_pdf = generate_shopping_list_pdf(final_combined)
        with open(shopping_pdf.name, "rb") as file:
            st.download_button("Download PDF", data=file, file_name="shopping_list.pdf", mime="application/pdf")
    ...
    def generate_recipe_guides(recipes_df):
        ...
        pdf.set_font("Arial", size=12)
        for recipe in selected_recipes:
            section = recipes_df[recipes_df["RecipeName"] == recipe].dropna(subset=["Method"])
    ...
    if st.button("Download Recipe Guides PDF"):
        recipe_pdf = generate_recipe_guides(filtered_df)
        with open(recipe_pdf.name, "rb") as file:
            st.download_button("Download PDF", data=file, file_name="recipe_guides.pdf", mime="application/pdf")
